...
func httpGetJSON(env Env, fullURL string) ([]byte, error) {
	...
	for i := 0; i < 3; i++ {
		...
		if resp.StatusCode != 200 {
			...
			resp.Body.Close()
			extractorAPIFetchDuration.Observe(time.Since(fetchStart).Seconds())
...
func fetchCrashesPage(env Env, job Job, offset int) (rows int, crashIDs []string, body []byte, maxUpdated time.Time, err error) {
	...
	rows = len(arr)
	// Record rows fetched
	extractorRowsFetched.Add(float64(rows))
...
func fetchOneBatchAllPages(env Env, mcli *minio.Client, job Job, corr string, ds *DatasetSpec,
	...
	for off := 0; ; off += limit {
		...
		// Record rows fetched for enrich datasets
		extractorRowsFetched.Add(float64(rows))
...
var (
	...
	extractorRunsTotal = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "extractor_runs_total",
		Help: "Total number of extract jobs executed",
	})
	extractorErrorsTotal = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "extractor_errors_total",
		Help: "Total number of extract job errors",
	})
	extractorDurationSeconds = prometheus.NewHistogram(prometheus.HistogramOpts{
		Name:    "extractor_duration_seconds",
		Help:    "Duration of extract job execution",
		Buckets: []float64{1, 5, 10, 30, 60, 120, 300, 600},
	})
	extractorRowsFetched = prometheus.NewCounter(prometheus.CounterOpts{
		Name: "extractor_rows_fetched_total",
		Help: "Total number of rows fetched from API",
	})
	extractorAPIFetchDuration = prometheus.NewHistogram(prometheus.HistogramOpts{
		Name:    "extractor_api_fetch_duration_seconds",
		Help:    "Duration of API fetch operations",
		Buckets: []float64{0.1, 0.5, 1, 2, 5, 10, 30},
	})
	extractorStartTime = time.Now()
)

func init() {
	// Register all metrics
	prometheus.MustRegister(extractorUptime)
	prometheus.MustRegister(extractorRunsTotal)
	prometheus.MustRegister(extractorErrorsTotal)
	prometheus.MustRegister(extractorDurationSeconds)
	prometheus.MustRegister(extractorRowsFetched)
	prometheus.MustRegister(extractorAPIFetchDuration)
}

func updateUptime() {
	for {
		extractorUptime.Set(time.Since(extractorStartTime).Seconds())
...
func main() {
	...
	go func() {
		...
		log.Printf("Prometheus metrics server started on port 8080")
	...
	for d := range msgs {
		jobStart := time.Now()
		extractorRunsTotal.Inc()
		
		var job Job
		if err := json.Unmarshal(d.Body, &job); err != nil {
			log.Printf("bad job json: %v", err)
			extractorErrorsTotal.Inc()
			extractorDurationSeconds.Observe(time.Since(jobStart).Seconds())
			d.Ack(false)
			continue
		}
		if job.Primary.ID == "" {
